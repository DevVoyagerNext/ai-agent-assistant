...
type UserSubjectProgress struct {
	...
	UserID          int       `gorm:"not null;uniqueIndex:uk_user_subject,priority:1;index:idx_user_status,priority:1;index:idx_user_time,priority:1;comment:用户ID" json:"userId"`
	SubjectID       int       `gorm:"not null;uniqueIndex:uk_user_subject,priority:2;comment:教材ID" json:"subjectId"`
	Status          string    `gorm:"type:enum('learning','completed');default:'learning';index:idx_user_status,priority:2;comment:整体状态：学习中，已学完" json:"status"`
